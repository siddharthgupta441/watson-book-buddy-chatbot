from flask import Flask, request, jsonify
from flask_cors import CORS
from dotenv import load_dotenv
import requests
from ibm_watson import AssistantV2
from ibm_cloud_sdk_core.authenticators import IAMAuthenticator

load_dotenv()
app = Flask(__name__)
CORS(app)

API_KEY = "4BSE3PNAs-gXJOLABgUm1Yj2Y5yLtTkG0RRqUlvrmIc9"
SERVICE_URL ="https://api.au-syd.assistant.watson.cloud.ibm.com/v2/assistants/be2dcb3b-11f9-468e-884a-83a710573f66/sessions"
ASSISTANT_ID = "be2dcb3b-11f9-468e-884a-83a710573f66"
ENV_ID = "draft"

print("API Key loaded:", API_KEY is not None)

def get_access_token():
    url = "https://iam.cloud.ibm.com/identity/token"
    headers = {
        "Content-Type": "application/x-www-form-urlencoded"
    }
    data = {
        "grant_type": "urn:ibm:params:oauth:grant-type:apikey",
        "apikey": API_KEY
    }

    response = requests.post(url, headers=headers, data=data)
    #print(response.json()["access_token"])
    return response.json()["access_token"]

@app.route("/chat", methods=["POST"])
def chat():
    user_message = request.json.get("message")
    access_token = get_access_token()
    # access_token = "eyJraWQiOiIyMDE5MDcyNCIsImFsZyI6IlJTMjU2In0..qx7MlFGWkFvJtGg-Kqh9CJ8vlk2wc_GMxG8PSP0fGrh7ZfocJXWZS6WOcgbf2dq-8RYczekINfv49W7ApnLHsacy4qbxJ_5j4Cj0bauQQ6509pg64K5p1JwLqSa3TcihExgg3IkNg7wvelmVtFTbf5nZezTKpQFvyaGkiDSbFKqss6vx5tXoB1TPbAE3ntuZ7cVv7RiGVSrVTNXMJfQmyAA-h_ER6uDAyPrwKtIagdfzFZWlXBQbirPzt9sapfkbE6-BPRmHQ2G9VUQIVFLIapiNLEpCnmX5sD3mYV9zvTt_VbWUZ42hSKd6TtTNo-VLNRHmwficCQnFa9ZtAy-9pQ"

    url = "https://api.au-syd.assistant.watson.cloud.ibm.com/v2/assistants/be2dcb3b-11f9-468e-884a-83a710573f66/environments/{ENV_ID}/message?version=2024-05-01"
    #url = f"{SERVICE_URL}/v2beta/projects/{ASSISTANT_ID}/environments/{ENV_ID}/message?version=2024-05-01"
    headers = {
        "Authorization": f"Bearer {access_token}",
        "Content-Type": "application/json"
    }

    payload = {
        "input": {
            "text": user_message
        }
    }

    response = requests.post(url, headers=headers, json=payload)
    res_json = response.json()
    #print("Calling:", url)
    #print("Response JSON:", res_json.get('output'))

    try:
        output_text = res_json['output']['generic'][0]['text']
    except (KeyError, IndexError):
        output_text = f"message:- {res_json.get('output')}, error:- {res_json.get('error')}"

    return jsonify({"reply": output_text})

if __name__ == "__main__":
    app.run(debug=True)